...
- Lucas Galle Melchior - RM: 574027
...
|---|---|---|
| **LangChain** | Orquestração do núcleo conversacional e memória por sessão | Framework de agentes exigido na Sprint 3; memória gerenciada nativamente via `RunnableWithMessageHistory` |
| **Groq (openai/gpt-oss-120b)** | Modelo de linguagem principal (produção) | Gratuito, respostas em ~2s em média, dentro da janela de timeout do webhook Twilio |
| **Google Gemini (gemini-3.6-flash)** | Modelo secundário, usado na comparação do Bloco B | Gratuito via Google AI Studio; ver `relatorio_modelos.md` — não usado em produção por latência incompatível com o timeout do webhook |
...
## Evolução — Sprint 3 (LangChain)

Na Sprint 3, o núcleo conversacional foi reconstruído sobre o **LangChain**, mantendo a mesma arquitetura RAG (ChromaDB) e o mesmo canal de produção (Flask + Twilio) descritos acima. O fluxo de funcionamento continua o mesmo em essência — a mudança está em como a memória de conversa é gerenciada.

**Principais mudanças:**
- **Memória por sessão gerenciada pelo framework** (`RunnableWithMessageHistory`), indexada por `session_id`, no lugar da lista Python manual usada nas Sprints 1/2.
- **Comparação entre modelos** (Groq vs. Google Gemini) — resultados, parâmetros e seleção justificada em [`relatorio_modelos.md`](./relatorio_modelos.md).
- **Testes de segurança / prompt injection** — 3 casos documentados (override direto, jailbreak por autoridade falsa, indução a aconselhamento fora do escopo), com 2 lacunas de guardrail identificadas e corrigidas no `SYSTEM_PROMPT`.
- Decisões técnicas, trade-offs e o comparativo antes/depois completo estão em [`relatorio_evolucao_sprint3.pdf`](./relatorio_evolucao_sprint3.pdf).

**Reproduzindo os testes:**
```bash
python compare_models.py   # roda o eval set nos 2 modelos, gera comparativo_raw.md
python test_seguranca.py   # roda os 3 casos de segurança, gera seguranca_raw.md
```
